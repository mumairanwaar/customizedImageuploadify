...
; (function ($, window, document, undefined) {
  ...
  $.fn.imageuploadify = function (opts) {
    ...
    this.each(function () {
      ...
      const retrieveFiles = (files, byPassTypeCheck) => {


        if (settings.laodingFilesStartCallback && typeof settings.beforeFileAddCallback == 'function') {
           settings.laodingFilesStartCallback();
        }
        ...
        for (let index = 0; index < files.length; ++index) {
          ...
          if (    byPassTypeCheck ||  (!accept || compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])    )) {
            // Unique number to save the image.
            const id = Math.random().toString(36).substr(2, 9);

            var addFile = true;
            if (settings.beforeFileAddCallback && typeof settings.beforeFileAddCallback == "function") {
              try {
                if (settings.beforeFileAddCallback(id, files[index], byPassTypeCheck) === false) {
                  addFile = false;
                }
              } catch (error) {
                console.log('beforeFileAddCallback throws an error:');
                console.log(error);
                addFile = false;
              }
        ...
        if (!settings.loadingFilesCompletedCallback && typeof settings.loadingFilesCompletedCallback == 'function') {
          settings.laodingFilesStartCallback();
        }
        ...
        deleteBtn.on("click", function () {
          ...
          for (let index = 0; totalFiles.length > index; ++index) {
            if (totalFiles[index].id === id) {
              var fileArg = totalFiles[index];
              totalFiles.splice(index, 1);
              //here u can call ur callback function 
              //on delete 
              if (settings.deletedCallback && typeof settings.deletedCallback == "function") {
                settings.deletedCallback(fileArg);
              }
              break;
            }
          }
      ...
      console.log(self);
      /**
       * @param {Array} files contain name property name of file to add
      */
      self.addFiles = function (files) {
        console.log('AddFilesManuallyCalled');
        console.log(files);
        debugger;
        retrieveFiles(files, true);
      }
